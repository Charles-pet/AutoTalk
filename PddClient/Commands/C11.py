...
def Dutch_pig(self, event=None):
    pyautogui.click(TextSendXY)
    str1='荷兰猪不能消化吸收淀粉，只能吃草，或者100%纯草颗粒粮'
    pyperclip.copy(str1)
    ...
    pyautogui.click(TextSendXY)
    str2='自身不能合成维生素，需要每周吃一次新鲜的蔬菜，补充维生素\n白菜青菜生菜包心菜都可以'
    pyperclip.copy(str2)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str3='荷兰猪没事爱舔毛，就是在自己洗澡，所以不用专门洗澡'
    pyperclip.copy(str3)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str4='如果一定要洗澡，水洗，水温40度左右比较好，洗完彻底搽干净，避免着凉。'
    pyperclip.copy(str4)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
