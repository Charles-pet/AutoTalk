...
def Recipe(self, event=None):

    pyautogui.click(TextSendXY)
    str1='亲，宠物每天吃自己体重（百分之3-5%）的食物。\n吃饱了就不吃了。'
    pyperclip.copy(str1)
    ...
    pyautogui.click(TextSendXY)
    str6='因为每个宠物品种、体重都不一样，每个月能吃多少可以简单的计算一下：\n体重*5%*30天，大概就是每个月能吃多少。'
    pyperclip.copy(str6)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str7='亲 兔粮、龙猫粮、荷兰猪粮=宠物吃的主食，\n苜蓿草、提摩西草、\n苜蓿草颗粒、提摩西草颗粒\n这些都是纯草粮，因为是纯草粮，所以是最健康的宠物主食，纯天然、绿色健康。'
    pyperclip.copy(str7)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    #插入第2张图片 
    filepath = r"C:\AutoTalk\PddClient\MiBookAir\Picture\Timothy\Timothym.jpg"
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
