...
def Unlimited_grazing(self, event=None):
    pyautogui.click(TextSendXY)
    str1='亲，可以吃，很合适。吃纯草或者纯草颗粒，都是最健康的食物。'
    pyperclip.copy(str1)
    ...
    Command_sendVideo('01') #发送视频介绍-兔子吃草
    Command_sendVideo('08') #发送视频介绍-荷兰猪吃草

    pyautogui.click(TextSendXY)
    str6='都是敞开吃草喝水，大吃大喝，都活得很健康很快乐。'
    pyperclip.copy(str6)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str7='吃饱了就不吃了，没听说吃的被自己撑住的，更没有听说过被自己撑死的。 ☺ '
    pyperclip.copy(str7)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    #发送提草质量照片
    filepath = r"C:\AutoTalk\PddClient\MiBookAir\Picture\Timothy\Timothyw.jpeg"
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
