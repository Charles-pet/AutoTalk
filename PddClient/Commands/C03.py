...
def Socializing(self, event=None):
    ...
    pyautogui.click(TextSendXY)
    str2='把小宠带回家时，由于它离开了一直生活的环境，离开了母亲和兄弟姐妹，会有比较长时间显得非常敏感、紧张是失落，有时会出现咬人，发呆，不肯吃东西等情况，而这个时期是你和宠物建立信任的关键时期，如果处理不好以后就很难再建立良好的关系了，有可能你的小宠会变成一只一直咬人的坏宠物！以下有个时间表，不要心急，一点点和你的小宠相处:'
    pyperclip.copy(str2)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str3='第1天:让它适应新的环境，完全不要骚扰它，不要碰它，更不要把它拿来拿去。'
    pyperclip.copy(str3)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str4='第2天:可用手喂它吃东西，轻声和它说话，让它熟悉你的气味和声音。'
    pyperclip.copy(str4)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str5='第5天:可将它轻轻抱起，放在手心（别让它摔落或跑丢）。'
    pyperclip.copy(str5)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str7='抱宠物时要注意:睡着的小宠不喜欢受到骚扰，所以当你要抱起它之前先肯定它是清醒的，应该先把它的窝拨开以便它可以看到你，然后再轻轻地将它抱起，如果它不愿意，千万不要勉强。'
    pyperclip.copy(str7)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
