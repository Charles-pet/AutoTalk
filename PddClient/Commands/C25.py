...
def NoEat(self, event=None):

    pyautogui.click(TextSendXY)
    str1='我看到有很多买家给我说正在给宠物吃蔬菜、胡萝卜、水果，\n甚至火腿肠、面包等等，这很危险。'
    pyperclip.copy(str1)
    ...
    pyautogui.click(TextSendXY)
    str4='水果和蔬菜都差不多，童话里说的兔子爱吃胡萝卜，只是童话，不适合吃，不友好。'
    pyperclip.copy(str4)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str5='火腿肠就更不能吃，吃草的动物，让吃肉？会引起严重腹泻。'
    pyperclip.copy(str5)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str6='面包也不能吃，都是淀粉，消化不了，积累在盲肠，引起肠炎，都是有害的。'
    pyperclip.copy(str6)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str7='给食草宠物吃:蔬菜、水果、面包、火腿肠等等，可能短期没啥问题，这是幸运，时间到了就挂掉了。'
    pyperclip.copy(str7)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str8='另外，一般的兔粮最好也别吃，不是草做的，淀粉太多，消化不了都积累在盲肠，引起肠炎，\n时间到了，宠物莫名其妙的挂了，就是这个原因。'
    pyperclip.copy(str8)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str9='总结一下:兔子、龙猫、荷兰猪这些食草物种在地球上已经生活了几百万年，一直是吃草的，整个肠胃、牙齿都已经适应吃草，给它吃非草类食物，短期吃应急饿不死还行，长期吃肯定是百病丛生。\n1.吃干燥的草或者干燥的草颗粒，\n2.单独喝水，无限喝水（凉白开）'
    pyperclip.copy(str9)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
