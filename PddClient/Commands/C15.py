...
def Coupons(self, event=None):
    pyautogui.click(TextSendXY)
    str1='亲，当天在我店累计购物满18元，送1元无门槛优惠券，\n全平台都能用，隔天用的。\n如果签收后有三张照片评价，无论好评差评一律送三元优惠券。\n都是平台活动，个人中心，优惠券，自动到账。'
    pyperclip.copy(str1)
    ...
    def send_to_clipboard(clip_type, data):
        ...
        time.sleep (0.5)
    #插入第一张图片
    filepath = r'C:\AutoTalk\PddClient\MiBookAir\Picture\praise\praise1.jpg'
    ...
    #插入第二张图片
    filepath = r'C:\AutoTalk\PddClient\MiBookAir\Picture\praise\praise2.jpg'
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
    ...
    #插入第三张图片
    filepath = r'C:\AutoTalk\PddClient\MiBookAir\Picture\praise\praise3.jpg'
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
