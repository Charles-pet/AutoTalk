...
def Wood_pellets(self, event=None):
    ...
    def send_to_clipboard(clip_type, data):
        ...
        time.sleep (0.5)
    filepath = r'C:\autotalk\PddClient\MiBookAir\Picture\Wood\wood1.jpg'
    ...
    #插入第二张图片
    filepath = r'C:\autotalk\PddClient\MiBookAir\Picture\Wood\wood2.jpg'
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
    ...
    #插入第三张图片
    filepath = r'C:\autotalk\PddClient\MiBookAir\Picture\Wood\wood3.jpg'
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
    ...
    #插入第四张图片
    filepath = r'C:\autotalk\PddClient\MiBookAir\Picture\Wood\wood4.jpg'
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
    ...
    #插入第五张图片
    filepath = r'C:\autotalk\PddClient\MiBookAir\Picture\Wood\wood5.jpg'
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
    ...
    pyautogui.click(TextSendXY)
    str4="不让尿尿的地方喷点醋。\n还有一种，是在标注领地，这种就没有好的办法了☺"
    pyperclip.copy(str4)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)

    Command_sendVideo('I-兔')                       #------宠物吃草颗粒----------#
