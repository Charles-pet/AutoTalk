...
def Old_grass(self, event=None):

    pyautogui.click(TextSendXY)
    str1='◆亲，质量都一样，因为有些经验丰富老客户很懂养宠物，是19、20年养的兔子\荷兰猪\龙猫，一直吃我家的19、20年的提草，\n口味习惯了，不想换。\n都是机器自动收割切段烘干的，\n卫生标准提升很多，适口性大幅提高。\n我们是低温冷库存放，经过长时间低温发酵，草的益生菌大幅增长，有害菌几乎都被消灭，营养更好，口感更佳。'
    pyperclip.copy(str1)
    ...
    def send_to_clipboard(clip_type, data):
        ...
        time.sleep (0.5)
    filepath = r"C:\AutoTalk\PddClient\MiBookAir\Picture\Timothy\Timothyo.jpg"
    ...
    #插入第二张图片
    filepath = r'C:\AutoTalk\PddClient\MiBookAir\Picture\Timothy\Timothyp.jpg'
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
    ...
    #插入第三张图片
    filepath = r'C:\AutoTalk\PddClient\MiBookAir\Picture\Timothy\Timothyq.jpg'
    image = Image.open(filepath)
    output = BytesIO()
    image.convert("RGB").save(output, "BMP")
    data = output.getvalue()[14:]
    ...
    pyautogui.click(TextSendXY)
    str1="亲，我们开店很多年，回头客1.8万人，\n买家各种要求都有，所以在我们工厂的专业冷库密封存放了一点19、20年的草，\n专门提供给老客户。"
    pyperclip.copy(str1)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (1.3)

    Command_sendVideo('01')                 #-发送视频-兔子吃纯草兔粮--#
    Command_end()                           #------通用结尾操作----------#
