...
def Not_eat_grass(self, event=None):

    pyautogui.click(TextSendXY)
    str1="亲，小宠物都有一个通病，喜欢挑食。新的口粮都有一定时间的适应期，每天更换百分之五，过几天就好了。 实在不行饿几天也就好了。不会饿坏的，和人一样，饿了就吃了，吃的更香。"
    pyperclip.copy(str1)
    ...
    pyautogui.click(TextSendXY)
    str2="◆对于个别难教育的，誓死反抗的刺头，买个西红柿，榨成汁，倒点汁拌进去，吃的可欢了，还能补充维生素"
    pyperclip.copy(str2)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter') 
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str4="◆兔子、荷兰猪、龙猫也有等级制，但是一开始它们认为和你是平等的，你必须教训它几次，它才会知道你是老大。"
    pyperclip.copy(str4)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str5="◆注意:一定不要给它别的任何吃的，和人一样，有好吃的就先吃好吃的。兔粮、蔬菜、水果，一律停止，就像你把小孩带到肯德基，他还会吃馒头吗？只给水和提摩西草或者提草颗粒纯草兔粮，三天见效。"
    pyperclip.copy(str5)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str5="◆你可以先给它吃一点蔬菜（白菜、青菜、生菜、油麦菜、青椒、彩椒都可以）开胃，只吃一点点新鲜的，因为宠物的肠胃和人不一样，不能吸收食物中的水分，新鲜的不能多吃，会拉西。以后的蔬菜都要晒干了再吃，放在阳台至少一天，干了再吃。"
    pyperclip.copy(str5)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str6="◆别的吃的都停掉，不给任何希望，每天只给20克草粮，吃不饱也饿不坏，过几天就改正过来了。"
    pyperclip.copy(str6)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
    ...
    pyautogui.click(TextSendXY)
    str7="◆过年的几天，我忘了喂我养的兔子、荷兰猪，它们饿的连塑料脚垫都吃了。"
    pyperclip.copy(str7)
    pyautogui.hotkey('ctrl','v')
    pyautogui.press('enter')
    time.sleep (0.3)
